<script>
    ...
    switch (data.type) {
      case 'welcome':
        ...
        if (!isMaster) {
          messages = [{
            id: messageId++,
            content: "Welcome to HumanCaptcha! Please prove that you're a human.",
            role: 'master',
            timestamp: new Date()
          }];
        }
  ...
  <div class="flex h-screen bg-neutral-900">
    ...
    <div class="flex flex-col flex-1 max-w-4xl {isMaster ? '' : 'mx-auto'} bg-neutral-800">
      <header class="flex-shrink-0 bg-neutral-900 p-4">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-sm text-neutral-400 m-0">
                {isMaster ? `Chatting with ${selectedClient?.name || 'No client selected'}` : 'Connected to HumanCaptcha'}
              </p>
            </div>
          </div>
      ...
      <div class="flex-1 overflow-y-auto bg-neutral-800" bind:this={messagesContainer}>
        <div class="h-12 w-full absolute bg-gradient-to-b from-neutral-900 to-transparen z-50"></div>
      ...
      {#if (!isMaster || selectedClient) && connectionStatus === 'connected'}
        <div class="flex-shrink-0 p-4 bg-neutral-800 border-t border-neutral-700">
          <div class="flex items-center gap-3 bg-neutral-700 rounded-xl p-3 border border-neutral-600 focus-within:border-rose-500 transition-colors">
            <textarea
            ...
            <button
              ...
              class="bg-rose-500 hover:bg-rose-600 rounded-full disabled:bg-neutral-600 disabled:cursor-not-allowed border-none p-2 text-white cursor-pointer flex items-center justify-center transition-all duration-200 hover:enabled:-translate-y-0.5 flex-shrink-0"
